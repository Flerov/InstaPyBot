...
def bot(api: InstagramAPI, PATH, follow, comments, blacklist, tag, n, comment, minlikes):
    if not follow:
        t = 30
    log_blacklist, log2_blacklist = [], []
    with open(PATH + 'Config/Log/Auto-Tag-Bot-Log.txt', 'r') as log, open(PATH + 'Config/Log/Feed-Bot-Log.txt', 'r') as log2:
    ...
    for i in range(0, n):
        try:
            ...
            with open(PATH + 'Config/Log/Auto-Tag-Bot-Log.txt', 'a') as log, open(PATH + 'Config/Log/Feed-Bot-Log.txt', 'a') as log2:
            ...
            set.color(set.RED)
            print("File 'Auto-Tag-Bot-Log.txt' and 'Feed-Bot-Log.txt' not found in '/Log/'!")
            return
