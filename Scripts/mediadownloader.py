...
def pbdownloader(api: InstagramAPI):
    ...
    if api.searchUsername(username):
        ...
        urllib.request.urlretrieve(url, 'Downloads/' + username + '.jpg')
        set.color(set.GREEN)
        print("\n-------------------\n"
...
def postdownloader(api: InstagramAPI):
    ...
    while True:
        ...
        if mediatype == 1:
            ...
            urllib.request.urlretrieve(url, 'Downloads/' + str(medianumber) + '.jpg')
            print("\n-------------------\n"
                  "Single Picture added to /Downloads/\n"
                  "-------------------")
        elif mediatype == 2:
            url = str(feed['items'][medianumber]['video_versions'][0]['url'])
            urllib.request.urlretrieve(url, 'Downloads/video_' + str(medianumber) + '.mp4')
            print("\n-------------------\n"
            ...
            for i in feed['items'][medianumber]['carousel_media']:
                ...
                urllib.request.urlretrieve(url, 'Downloads/' + feed['items'][medianumber]['user'][
