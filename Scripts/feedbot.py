...
def autoprogressfeed(api: InstagramAPI, PATH, comments, blacklist, maxid=''):
    set.color(set.RED)
    print("\n-------------------------\n"
          "Sleeping 40sec between every interaction! (ban-protection)\n"
          "-------------------------\n")
    log_blacklist = []
    with open(PATH + 'Config/Log/Feed-Bot-Log.txt', 'r') as log:
    ...
    for i in data['items']:
        try:
            ...
            if i['user']['username'] not in blacklist and str(i['caption']['media_id']) not in log_blacklist:
                with open(PATH + 'Config/Log/Feed-Bot-Log.txt', 'a') as log:
                    ...
                    set.color(set.GREEN)
                    if '200' in str(api.LastResponse):
            ...
            set.color(set.RED)
            print("File 'Feed-Bot-Log.txt' not found in 'Config/Log/' !")
